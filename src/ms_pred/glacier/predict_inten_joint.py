...
def predict():
    ...
    with torch.inference_mode():
        for batch in tqdm(pred_loader):    
            ...
            collision_energies = batch["collision_engs"]

            outputs = pred_obj["spec"].cpu().numpy()
            for spec, collision_energy, output_spec in zip(
                spec_names, collision_energies, outputs
            ):
                output_obj = {
                    "spec_name": spec,
