...
from ms_pred.scarf_pred import scarf_data, scarf_model, train_gen
...
def score_function(config, base_args, orig_dir=""):
    ...
    train_dataset = scarf_data.ScarfDataset(
        df=train_df,
        data_dir=data_dir,
    ...
    val_dataset = scarf_data.ScarfDataset(
        df=val_df,
        data_dir=data_dir,
        graph_featurizer=graph_featurizer,
        file_map=subform_map,
    ...
    model = scarf_model.ScarfNet(
        ...
        use_tbc=kwargs.get("use_tbc"),
        info_join=kwargs.get("info_join"),
        root_embedder=kwargs["root_embedder"],
...
def get_args():
    ...
    train_gen.add_scarf_train_args(parser)
...
def get_param_space(trial):
    ...
    trial.suggest_categorical("use_tbc", [True])
