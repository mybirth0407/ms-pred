...
from ms_pred.scarf_pred import scarf_data, scarf_model, train_inten
...
def score_function(config, base_args, orig_dir=""):
    ...
    formula_folder = Path(kwargs.get("formula_folder"))

    all_json_pths = [Path(i) for i in formula_folder.glob("*.json")]
    subform_map = {i.stem.replace("pred_", ""): i for i in all_json_pths}
...
def get_param_space(trial):
    ...
    trial.suggest_int("mlp_layers", 1, 3)
    trial.suggest_int("set_layers", 0, 2)
    # trial.suggest_int("set_layers", 0, 0)
    trial.suggest_int("form_set_layers", 0, 3)
    ...
    trial.suggest_categorical("mpnn_type", ["GINE", "PNA", "GGNN"])
    trial.suggest_categorical("pool_op", ["avg", "attn"])
    trial.suggest_categorical("embed_adduct", [True])
    trial.suggest_categorical("info_join", ["concat", "attn"])
...
def get_initial_points() -> List[Dict]:
    ...
    return [init_base, init_base_1, init_base_2, init_base_3]
